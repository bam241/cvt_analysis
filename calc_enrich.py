...
def calc_del_U(v_a, Z, d, F_m, T, cut, eff=1.0, verbose=False):
    ...
    r_12 = np.sqrt(
        1.0 - (2.0 * R * T * (np.log(x)) / M / (v_a**2)))  # fraction
...
def design_cascade(cut, alpha, del_U, Nfc, feed_flows,
    ...
    for stage_idx in range(ideal_enrich_stage):
        ...
        if (verbose == True):
            print(stage_idx, "\t", n_mach_enr, "\t", round(Fs * fix, print_len), " ",
                  round(Ps * fix, print_len), "  ", round(Ws *
                                                          fix, print_len), " ",
    ...
    for stage_idx in range(ideal_strip_stage - 1, -1, -1):
        ...
        if (verbose == True):
            print((curr_stage), "\t", n_mach_strip, "\t",
                  round(Fs * fix, print_len), "  ", round(Ps *
                                                          fix, print_len), " ",
                  round(Ws * fix, print_len), "  ", round(Nfs, assay_len),
